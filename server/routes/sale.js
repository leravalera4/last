...
//     const city = Object.keys(cityEntry)[0];
//     const locations = cityEntry[city].locations;
//     Object.keys(locations).forEach(storeName => storeSet.add(storeName));
//   });
...
//       Object.assign(allLocations, locations[storeID]);
//     }
//   });
...
// router.get("/stores/:storeID/:city", (req, res) => {
//   const { storeID, city } = req.params;
//   const cityEntry = storesData.find(entry => Object.keys(entry)[0] === city);

//   if (cityEntry) {
//     const locations = cityEntry[city].locations;
//     const storeLocations = locations[storeID];

//     if (storeLocations) {
//       res.json({ city, locations: storeLocations });
//     } else {
//       res.status(404).json({ error: "Store not found in this city" });
//     }
//   } else {
//     res.status(404).json({ error: "City not found" });
//   }
...
// router.get("/stores/:storeID/:city/:id", (req, res) => {
//   const { storeID, city, id } = req.params;
//   const cityEntry = storesData.find(entry => Object.keys(entry)[0] === city);
...
//     if (storeLocations) {
//       const addressEntry = Object.entries(storeLocations).find(
//         ([address, addressId]) => addressId === id
//       );

//       if (addressEntry) {
//         const [address, addressId] = addressEntry;
//         res.json({ storeID, city, address, id: addressId });
//       } else {
//         res.status(404).json({ error: "Location ID not found" });
//       }
  ...
  try {
    for (const selectedStore of selectedStoresID) {
      ...
      const query = `
              ...
              WHERE "storeID" = $1 AND saleprice is NOT NULL and non_member_price is NULL AND stock IS NULL;
            `;
    ...
    const transformedObject = Object.entries(groupedByStoreID).map(
      ([key, value]) => {
        ...
        const processedItems = value.map((item) => {
          ...
          console.log("NON MEMBER PRICE",non_member_price)
          ...
          if (wasprice != null && saleprice != null) {
            ...
            console.log("DIFFERENCE_WAS", difference);
          }

          else if (non_member_price != null && saleprice != null) {
            difference = ((saleprice * quantity) - (non_member_price * quantity)) ;
            console.log("MEMBER_PRICE", difference);
            console.log("DIFFERENCE_WAS_NOT", difference);
          }
        ...
        const grouped = processedItems.reduce((acc, item) => {
          ...
          const val = quantity >= 1 && prices === 0 && stock != 'Out of Stock' ? 'Not sold here' : null
          if (!acc[productID]) {
            acc[productID] = {
              productID,
              title,
         ...
         if (
          ...
          //acc[productID].quantity += quantity; // Увеличиваем количество
          acc[productID].difference = 0;
          acc[productID].difference +=  ((acc[productID].non_member_price * acc[productID].quantity)-(acc[productID].saleprice * acc[productID].quantity))
          console.log("FINAL difference", acc[productID].difference);
        }

        if (
          acc[productID].wasprice > acc[productID].saleprice
        ) {
          console.log("MIMIMI");
          console.log("NON_MEMBER", acc[productID].wasprice);
          console.log("SALE_PRICE", acc[productID].saleprice);

          //acc[productID].quantity += quantity; // Увеличиваем количество
          acc[productID].difference = 0;
          acc[productID].difference +=  ((acc[productID].wasprice * acc[productID].quantity)-(acc[productID].saleprice * acc[productID].quantity))
          console.log("FINAL difference for WAS", acc[productID].difference);
        }
          ...
          //     acc[productID].saleprice * acc[productID].quantity;
          //   acc[productID].prices = totalSalePrice / 2; // Целевой расчет для prices
          //   acc[productID].difference = (acc[productID].non_member_price * acc[productID].quantity) - acc[productID].prices;
          //   console.log("PRICES_1", acc[productID].prices); // Выводим итоговую цену
          //   console.log("DIFFERENCE", acc[productID].difference); // Выводим итоговую цену
          // }
...
//          items: Object.values(grouped).sort((a, b) => {
//   const aOut = a.stock === 'Out of Stock' || a.val === 'Not sold here';
//   const bOut = b.stock === 'Out of Stock' || b.val === 'Not sold here';
//   return aOut - bOut; // false (0) < true (1), поэтому "Out" будут в конце
// }),
